...
func NewMessagingClient(hostname, port, username, password string) IMessagingClient {
	messagingClient := &MessagingClient{}

	connectionString := fmt.Sprintf("amqp://%v:%v@%v:%v", username, password, hostname, port)
	messagingClient.ConnectToBroker(connectionString)
...
func (m *MessagingClient) SubscribeToQueue(queueName string, consumerName string, handlerFunc func(amqp.Delivery)) error {
	...
	log.Debugf("Declaring Queue (%s)", queueName)
	queue, err := ch.QueueDeclare(
